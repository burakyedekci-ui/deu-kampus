/************************************************************
 ...
 Fakülte bazlı NDVI + GLCM Contrast
************************************************************/
...
// 4) NDVI
// ==========================================================
var ndvi = goruntu.normalizedDifference(['B8', 'B4']).rename('ndvi').clip(kampus);

// ==========================================================
// 5) GLCM İÇİN NDVI'YI TAMSAYIYA ÇEVİR
// ==========================================================
var ndviScaled = ndvi
  .add(1)
  .multiply(100)
...
// 6) GLCM TEXTURE
// ==========================================================
var glcm = ndviScaled.glcmTexture({size: 3, average: true});
var contrast = glcm.select('ndvi_int_contrast').rename('contrast');

// ==========================================================
// 7) HARİTADA CONTRAST GÖSTER
// ==========================================================
Map.addLayer(
  contrast,
  {min: 0, max: 150, palette: ['000000', '444444', '888888', 'dddddd', 'ffffff']},
  'GLCM Contrast'
);

// ==========================================================
// 8) FAKÜLTE BAZINDA ORTALAMA CONTRAST
// ==========================================================
var fakulte_contrast_ozet = fakulteler.map(function(f) {
  var stats = contrast.reduceRegion({
  ...
  return ee.Feature(null, {
    ...
    ortalama_contrast: stats.get('contrast')
  });
});

print('Fakulte bazli contrast ozet:', fakulte_contrast_ozet);

// ==========================================================
// 9) HER PİKSEL İÇİN CONTRAST TABLOSU
// ==========================================================
var contrast_samples = contrast.sampleRegions({
...
var contrast_piksel_tablo = contrast_samples.map(function(f) {
...
print('Contrast piksel tablosu ilk 10 satir:', contrast_piksel_tablo.limit(10));

// ==========================================================
// 10) EXPORTLAR
// ==========================================================
Export.table.toDrive({
  collection: fakulte_contrast_ozet,
  description: 'DEU_Fakulte_GLCM_Contrast_Ozet_2024',
  folder: 'GEE_Exports',
  fileNamePrefix: 'fakulte_glcm_contrast_ozet',
  fileFormat: 'CSV',
  selectors: ['fakulte_adi', 'ortalama_contrast']
});

Export.table.toDrive({
  collection: contrast_piksel_tablo,
  description: 'DEU_Fakulte_GLCM_Contrast_Piksel_2024',
  folder: 'GEE_Exports',
  fileNamePrefix: 'piksel_glcm_contrast',
  fileFormat: 'CSV',
  selectors: ['longitude', 'latitude', 'contrast', 'fakulte']
});

Export.image.toDrive({
  image: contrast,
  description: 'DEU_Fakulte_GLCM_Contrast_Raster_2024',
  folder: 'GEE_Exports',
  fileNamePrefix: 'deu_fakulte_glcm_contrast_raster_2024',
  region: kampus,
  scale: 10,
  maxPixels: 1e13
});
